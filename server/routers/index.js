...
const FpModel = require('../models/FpModel')
...
// 取消预约
// yyfw_id：房子id
router.post('/manage/cancle', (req, res) => {
  const { _id ,yyfw_id} = req.body
  FpModel.deleteOne({ _id })
    .then(cancle => {
      ProductModel.findOneAndUpdate({ _id: yyfw_id }, { orderstatus: 0 }).then(_=>{
        res.send({ status: 0, data: cancle })
      })
...
router.get('/manage/product/list', (req, res) => {
  ...
  ProductModel.find().sort({ "_id": -1 })
    ...
    .catch(error => {
      ...
      res.send({ status: 1, msg: '获取异常, 请重新尝试' })
    })
})
// 获取分配分页列表
router.get('/manage/fenpei/list', (req, res) => {
  const { pageNum, pageSize } = req.query
  // ProductModel.find().sort({"_id": -1}).limit(Number(pageSize))
  FpModel.find().sort({ "_id": -1 })
    .then(products => {
      res.send({ status: 0, data: pageFilter(products, pageNum, pageSize) })
    })
    .catch(error => {
...
// 获取销售员信息列表
router.get('/manage/sale/list', (req, res) => {
...
// 为申请人分配销售员
/* 
 // UserModel.updateMany({ _id: yhId }, {$set:{ yyfw_id: productId, yuyue_time: time }},{multi:true}).then(_ => {
...
router.get('/manage/product/search', (req, res) => {
  ...
  ProductModel.find(contition)
    ...
    .catch(error => {
      ...
      res.send({ status: 1, msg: '搜索异常, 请重新尝试' })
    })
})
